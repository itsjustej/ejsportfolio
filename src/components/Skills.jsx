      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Middle Frameworks Box */}
          <div className="bg-white rounded-lg p-8 shadow-md transform transition-transform hover:scale-105">
            ...
            <div className="space-y-6">
              {softSkills.map((frameWorks) => (
                <div key={frameWorks.name}>
                  <div className="flex justify-between mb-2">
                    <span className="font-medium text-slate-700">{frameWorks.name}</span>
                    <span className="text-slate-500">{frameWorks.level}%</span>
                  </div>
                  <div className="w-full bg-slate-200 rounded-full h-2.5">
                    <div 
                      className="bg-gradient-to-r from-purple-600 to-pink-300 h-2.5 rounded-full transition-all duration-1000 ease-out" 
                      style={{ width: `${frameWorks.level}%` }}
                    ></div>
                  </div>
                </div>
              ))}
